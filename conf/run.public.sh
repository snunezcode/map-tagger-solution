...
aws cloudformation create-stack --stack-name "map-tagger-solution-$id" --template-body file://MAPTaggerSolution.template --parameters ParameterKey=Username,ParameterValue=snmatus@amazon.com ParameterKey=VPCParam,ParameterValue=vpc-07d80a425057895a3 ParameterKey=SubnetParam,ParameterValue=subnet-03bff4b2b43b0d393 ParameterKey=InstanceType,ParameterValue=t3a.medium ParameterKey=PublicAccess,ParameterValue=true ParameterKey=SGInboundAccess,ParameterValue=0.0.0.0/0 ParameterKey=GitHubRepository,ParameterValue=aws-samples --region us-east-1 --capabilities CAPABILITY_NAMED_IAM
aws cloudformation wait stack-create-complete --stack-name "da-top-solution-$id" --region us-east-1
